...
namespace PepperDash.Essentials
{
	public class MobileControlSystemController : EssentialsDevice, IMobileControl
	{
		...
		private void SetupDefaultRoomMessengers()
		{
			...
			{
				Debug.LogMessage(LogEventLevel.Verbose, "Setting up room messengers for room: {key}", this, room.Key);
				...
				Debug.LogMessage(LogEventLevel.Verbose, "Attempting to set up default room messengers for room: {0}", this, room.Key);
		...
		private void SetupDefaultDeviceMessengers()
		{
			...
			Debug.LogMessage(LogEventLevel.Verbose, "All Devices that aren't rooms count: {0}", this, allDevices?.Count());
			var count = allDevices.Count();
			foreach (var device in allDevices)
			{
				try
				{
					Debug.LogMessage(LogEventLevel.Verbose, "Attempting to set up device messengers for device: {0}", this, device.Key);
					...
					{
						Debug.LogMessage(LogEventLevel.Verbose, "Trying to cast to ICommunicationMonitor for device: {0}", this, device.Key);
						var commMonitor = device as ICommunicationMonitor;
						if (commMonitor == null)
						{
							Debug.LogMessage(LogEventLevel.Debug, "[Error] CommunicationMonitor cast is null for device: {0}. Skipping CommunicationMonitorMessenger", this, device.Key);
							Debug.LogMessage(LogEventLevel.Debug, "AllDevices Completed a device. Devices Left: {0}", this, --count);
							continue;
						}
						Debug.LogMessage(LogEventLevel.Debug, "Adding CommunicationMonitorMessenger for device: {0}", this, device.Key);
						var commMessenger = new ICommunicationMonitorMessenger($"{device.Key}-commMonitor-{Key}",
							string.Format("/device/{0}", device.Key), commMonitor);
					...
					{
						Debug.LogMessage(LogEventLevel.Verbose, "Trying to cast to ITvPresetsProvider for device: {0}", this, device.Key);
					...
					if (device is ISetTopBoxControls)
					{
						...
						var messenger = new ISetTopBoxControlsMessenger($"{device.Key}-stb-{Key}", $"/device/{device.Key}", dev);
					...
					if (device is IChannel)
					{
						...
						var messenger = new IChannelMessenger($"{device.Key}-channel-{Key}", $"/device/{device.Key}", dev);
					...
					if (device is IColor)
					{
						...
						var messenger = new IColorMessenger($"{device.Key}-color-{Key}", $"/device/{device.Key}", dev);
					...
					if (device is IDPad)
					{
						...
						var messenger = new IDPadMessenger($"{device.Key}-dPad-{Key}", $"/device/{device.Key}", dev);
					...
					if (device is INumericKeypad)
					{
						...
						var messenger = new INumericKeypadMessenger($"{device.Key}-numericKeypad-{Key}", $"/device/{device.Key}", dev);
					...
					if (device is IHasPowerControl)
					{
						...
						var messenger = new IHasPowerMessenger($"{device.Key}-powerControl-{Key}", $"/device/{device.Key}", dev);
					...
					if (device is ITransport)
					{
						...
						var messenger = new IChannelMessenger($"{device.Key}-transport-{Key}", $"/device/{device.Key}", dev);
					...
					Debug.LogMessage(LogEventLevel.Verbose, "Trying to cast to generic device for device: {key}", this, device.Key);
					if (device is EssentialsDevice)
					{
						var genericDevice = device as EssentialsDevice;
						if (genericDevice == null || messengerAdded)
						{
							Debug.LogMessage(LogEventLevel.Verbose, "Skipping GenericMessenger for device: {0}. Messenger Added: {1}. GenericDevice null: {2}", this, device.Key, messengerAdded, genericDevice == null);
							Debug.LogMessage(LogEventLevel.Debug, "AllDevices Completed a device. Devices Left: {0}", this, --count);
							continue;
						}
						Debug.LogMessage(LogEventLevel.Debug, "Adding GenericMessenger for device: {0}", this, genericDevice?.Key);
					...
					{
						Debug.LogMessage(LogEventLevel.Verbose, "Not Essentials Device. Skipping GenericMessenger for device: {0}", this, device.Key);
					}
					Debug.LogMessage(LogEventLevel.Debug, "AllDevices Completed a device. Devices Left: {0}", this, --count);
				...
				{
					Debug.LogMessage(LogEventLevel.Verbose, "[ERROR] setting up default device messengers: {0}", this, ex.Message);
